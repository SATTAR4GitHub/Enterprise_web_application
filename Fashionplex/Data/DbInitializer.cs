...
namespace Fashionplex.Data
{
    public static class DbInitializer
    {
        ...
        private static async Task<int> SeedUsers(UserManager<ApplicationUser> userManager)
        {
            ...
            {
                UserName = "manager@fashionplex.ca",
                Email = "manager@fashionplex.ca",
                FirstName = "Mr.",
                LastName = "Manager",
                EmailConfirmed = true
            };

            var result = await userManager.CreateAsync(managerUser, "Manager@100");
            if (!result.Succeeded)
                return 1;  // should log an error message here
            ...
            {
                UserName = "admin@fashionplex.ca",
                Email = "admin@fashionplex.ca",
                FirstName = "Mr.",
                LastName = "Admin",
                EmailConfirmed = true
            };

            result = await userManager.CreateAsync(adminUser, "Admin@100");
            if (!result.Succeeded)
                return 3;  // should log an error message here
